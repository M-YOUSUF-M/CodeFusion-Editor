...
class Editor(QsciScintilla): # Defining a class Editor that inherits from QsciScintilla for code editing.
    ...
    def run_program(self, file_path): # Method to run a program.

        """Run the program based on its file type."""

        file_path = self.file_path # Getting the file path.

        if file_path: # Checking if the file path is not empty.

            with open(file_path, 'w') as file: # Opening the file in write mode.
        ...
        _, file_extension = os.path.splitext(file_path) # Getting the file extension.
    ...
    # Determine action based on file type
        ...
        if file_extension == ".py": # Checking if the file is a Python file.
            ...
            if self.is_executable_available("python"): # Checking if Python is installed.

                print("Python interpreter found. Running the script...") # Printing a message to the console.
        ...
        elif file_extension in [".c", ".cpp"]: # Checking if the file is a C or C++ file.

            # C or C++ file

            compiler = "gcc" if file_extension == ".c" else "g++" # Setting the compiler based on the file extension.

            if self.is_executable_available(compiler): # Checking if the compiler is installed.

                print(

                   f"{compiler} compiler found. Compiling and running the program...") # Printing a message to the console.

            # Compile the program

                output_file = os.path.join(os.getcwd(), _) # Setting the output file name.

                compile_command = [compiler, file_path, "-o", output_file] # Creating the compile command.

                # execute_command_in_terminal(compile_command) #commented out: Unnecessary function call.

                compile_process = subprocess.run(compile_command) # Running the compile command.



            # Check if compilation was successful

                if compile_process.returncode == 0: # Checking if the compilation was successful.
                ...
                # Clean up the compiled output file after running
                   ...
                   print("Compilation failed.") # Printing an error message to the console.
